...
// the provenance audit, check-main-provenance.mjs, reports a commit above its
// trailer baseline that came through a pull request without it. That is how a
// merge that went around this script is seen after the fact: `gh pr merge`, a
// GraphQL mutation, the button in the GitHub UI. The merge guard is
// prevention, and prevention that is bypassed says nothing; this is detection
// on the result.
//
// It catches an accident, not an adversary. Anyone can type the trailer into a
// merge by hand and the audit cannot tell. The incident it answers was an
// agent merging by mistake, which is the case worth catching.
//
// Passing a message replaces GitHub's default one wholesale, as `gh pr merge
// --squash --body` does. So this rebuilds what the default holds under the
