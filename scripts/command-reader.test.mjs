...
for (const [file, m] of Object.entries(modules['uncommitted work'])) {
  test(`${file}'s uncommitted work rule knows a destructive command from its neighbours`, () => {
    ...
    assert.deepEqual(read('reset --hard origin/main'), { lose: 'all', paths: [] })
    assert.equal(read('reset --soft HEAD~1'), null)
    assert.deepEqual(read('checkout -f'), { lose: 'all', paths: [] })
    assert.deepEqual(read('checkout -qf main'), { lose: 'all', paths: [] })
    ...
    assert.deepEqual(read('switch -f main'), { lose: 'all', paths: [] })
