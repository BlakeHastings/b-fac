// The two guards carry one command reader between them, in two copies.
//
...
// two guards' rules genuinely differ and are meant to. #98 measured `\git
// push`, `/usr/bin/gh pr create` and `git.exe push` denied by the guest gate
// and allowed by the merge guard, because every rule there goes through
// `commandName` while `ghArguments` in the merge guard compares the raw token.
// Asserting equal verdicts would be asserting a fiction. Asserting equal
// segmentation is asserting the thing that is actually one thing.
//
...
// this repository, out of the two files that ship. Whether the duplication
// should end is #93's question and this does not answer it.
//
//   npm test
...
const [FIRST, SECOND] = Object.keys(GUARDS)
...
test('both guards carry the same reader, comments aside', () => {
  const first = codeLines(readerSource(GUARDS[FIRST]))
  const second = codeLines(readerSource(GUARDS[SECOND]))
  assert.equal(first.length > 50, true, 'the marked region is too small to be the reader')
  assert.deepEqual(
    second,
    first,
    `The command reader has drifted between ${FIRST} and ${SECOND}.\n` +
      'Both copies answer the same question and a fix belongs in both, in one\n' +
      'commit. ADR 0029 refuses a shared module; #93 holds the duplication.',
  )
})

// A corpus of lines chosen for how they *segment*, not for what either guard
// then decides about them. Half of these are allowed by both guards; that is
// fine, because the assertion is about the tokens.
const CORPUS = [
...
for (const line of CORPUS) {
  test(`both readers segment alike: ${JSON.stringify(line)}`, () => {
    assert.deepEqual(readers[SECOND].segmentsOf(line), readers[FIRST].segmentsOf(line))
  })
}

// Agreement alone is satisfied by two readers that are wrong in the same way,
