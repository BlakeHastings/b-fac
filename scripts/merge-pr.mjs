...
// button cannot.
//
// THIS IS THE SHIPPED ASSET, PLUS TWO THINGS
// The body is .agents/skills/orchestrated-delivery/assets/merge-pr.mjs, run
// here so that its logic is exercised somewhere before a stranger trusts it.
// The copies differ in REQUIRED's values, in comments, and in WHAT JUST
// SHIPPED below, which only means something where the payload is a plugin.
// Anything else that differs is drift. ADR 0063 says which difference is
// which, and scripts/merge-pr.test.mjs runs one suite against both, so drift
// in what the shared body does fails a test rather than waiting for a review.
//
// WHAT THIS PREVENTS
...
// branch". So a stale branch gets a refusal that names the fix instead of a
// raw 405.
//
...
// repository with no ruleset, and for a lookup that fails. Here the ruleset
// answers, so this matters only when gh cannot reach it; keep it in step with
// .github/workflows/checks.yml anyway, because that is when it is read.
...
//
// On here although the ruleset already refuses a stale branch: redundant in
// this repository, and running here is the only exercise it gets.
...
export async function run({
  ...
  const versionBefore = shippedVersion(gh, base)
  ...
  const versionAfter = shippedVersion(gh, base)
  if (versionBefore === null || versionAfter === null) {
    log(`\nCould not read ${MANIFEST} on ${base}, so this cannot say whether the`)
    log('merge moved the shipped version. The merge itself succeeded.')
  } else if (versionAfter !== versionBefore) {
    log(`\nReleased: ${base} now ships ${versionAfter}, was ${versionBefore}.`)
    log('Installers get it on their next `claude plugin update`. That is the release,')
    log('and no tag is owed for it — docs/process/releasing.md says when one is.')
  }
...
// twice, once before the merge and once after, because the difference between
// those two answers is exactly "did this merge release anything". Absent from
// the shipped asset, whose host repositories have no plugin.json.
const MANIFEST = '.claude-plugin/plugin.json'

function shippedVersion(gh, ref) {
  try {
    // Raw media type rather than the default base64 envelope: the answer is a
    // small JSON file and decoding it twice buys nothing.
    return JSON.parse(
      gh(['api', '-H', 'Accept: application/vnd.github.raw', `repos/{owner}/{repo}/contents/${MANIFEST}?ref=${ref}`]),
    ).version
  } catch {
